...
function NotificationDrawer({ open, onClose }) {
  ...
  useRealtimeEvent("notification.new", () => { if (open) load(); });
...
export default function Layout() {
  ...
  useRealtimeEvent("transaction.updated", async (p) => {
    ...
    try {
      ...
      if (tx.borrower_id !== user.id) return;
      const { data: elig } = await api.get(`/ratings/transaction/${tx.id}/eligibility`);
      if (!elig.allowed || elig.already_rated) return;
      setRatingPrompt({
        transactionId: tx.id,
  ...
  if (user?.is_admin) tabs.push({ to: "/admin", label: "Admin", icon: ShieldCheck });
