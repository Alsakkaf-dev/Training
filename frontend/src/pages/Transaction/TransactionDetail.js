...
export default function TransactionDetail() {
  ...
  const report = async () => {
    const description = window.prompt("Briefly describe the problem:");
    if (description === null) return;
    try {
      await api.post(`/chat/by-transaction/${id}/report`, { report_category: "Harassment_Abuse", description });
      ...
      <div className="space-y-3" data-testid="tx-actions">
        ...
        {(chatOpen || (isBorrower && ["Pending", "Approved"].includes(tx.status))) && (
          <div className="flex gap-2.5">
            ...
            {chatOpen && (
              <Button variant="secondary" className="flex-1 !py-3 !text-status-cancelled hover:!border-red-300" onClick={report} data-testid="tx-report">
                <Flag size={16} /> Report
              </Button>
            )}
          </div>
