...
import { Button, Input, Modal, PageLoader, StarRating, Avatar, StatusBadge, TrustRing, ProgressBar } from "../../components/ui";
...
export default function Profile() {
  ...
  const save = async () => {
    setBusy(true); setErr("");
    try {
      const { data } = await api.put("/profile", { phone_number: phone, profile_picture: photo });
      setUser({ ...user, phone_number: data.user.phone_number, profile_picture: data.user.profile_picture });
      setEditOpen(false);
