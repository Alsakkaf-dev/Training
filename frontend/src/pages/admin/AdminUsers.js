...
export default function AdminUsers() {
  ...
  const remove = async (u) => {
    if (!window.confirm(
      `Permanently delete ${u.full_name}?\n\nThis wipes the user, ALL their listings, and every transaction they were part of. Cannot be undone.`
    )) return;
    setBusy(u.id);
    try {
  ...
  const bulkSuspend = async () => {
    if (!selected.length || !window.confirm(`Suspend ${selected.length} users?`)) return;
    setBulkBusy(true);
    let ok = 0;
  ...
  const bulkDelete = async () => {
    if (!selected.length || !window.confirm(`Permanently delete ${selected.length} users? This cannot be undone.`)) return;
    setBulkBusy(true);
    let ok = 0;
    for (const id of selected) {
