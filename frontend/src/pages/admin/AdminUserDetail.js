...
export default function AdminUserDetail() {
  ...
  const deleteUser = async () => {
    if (!window.confirm(`Permanently delete this user? This wipes all their data and cannot be undone.`)) return;
    setBusy(true);
    try {
      ...
      <Card className="p-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            <Avatar name={u.full_name} src={u.profile_picture} size={56} />
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="font-head font-extrabold text-xl text-ink">{u.full_name}</h1>
                {isAdmin && <ShieldCheck size={18} weight="fill" className="text-brand-500" />}
              </div>
          ...
          {/* Sticky actions — full-width on mobile */}
