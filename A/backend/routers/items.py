...
async def edit_item(item_id: str, body: ItemIn, user: dict = Depends(get_current_user)):
    ...
    if it["availability_status"] in ("Pending", "Borrowed"):
        raise HTTPException(status_code=400, detail="Cannot edit an item that is currently requested or borrowed.")
...
async def delete_item(item_id: str, user: dict = Depends(get_current_user)):
    ...
    if it["availability_status"] in ("Pending", "Approved", "Borrowed"):
        raise HTTPException(
            status_code=400,
            detail="Cannot delete an item with an active borrow request or loan.",
        )
